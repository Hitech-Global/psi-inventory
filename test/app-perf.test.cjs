...
test('LOG-3: 精准 invalidation — notify 不脏；backfill-arrival / listing 仅脏 logistics 列表页', async () => {
  ...
  // backfill-arrival（修改时效/到货状态，LIST 展示列）→ 脏 logistics 列表页 + 物流决策 summary
  // （物流决策的成本/时效口径直接读 logistics_batches，到货日期回填会改变 P50/P90），
  // 但不脏费用四页/在途族。
  const LOGI_LIST_PAGES = ['logistics', 'logistics-decision-summary'];
  const m2 = A._internal.pagesForMutation('/api/logistics-batches/abc/backfill-arrival', 'POST');
  assert.deepStrictEqual(m2.pages, LOGI_LIST_PAGES, 'backfill-arrival 应仅脏物流列表页+物流决策，实际=' + JSON.stringify(m2.pages));
  // listing（变更上架状态，LIST 展示列）→ 同上
  const m3 = A._internal.pagesForMutation('/api/logistics-batches/abc/listing', 'POST');
  assert.deepStrictEqual(m3.pages, LOGI_LIST_PAGES, 'listing 应仅脏物流列表页+物流决策，实际=' + JSON.stringify(m3.pages));
});
