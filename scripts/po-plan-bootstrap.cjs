...
const ASSET_VERSION='20260917c';
const UI_MARKER='<script src="po-plan-ui.js?v='+ASSET_VERSION+'"></script>';
const DRAFT_SYNC_MARKER='<script src="po-plan-draft-sync.js?v='+ASSET_VERSION+'"></script>';
const UI_BLOCK=UI_MARKER+'\n'+DRAFT_SYNC_MARKER;
...
function patchIndex(src){let out=src.replace(/\s*<script src="po-plan-ui\.js(?:\?[^\"]*)?"><\/script>/g,'').replace(/\s*<script src="po-plan-draft-sync\.js(?:\?[^\"]*)?"><\/script>/g,'');return once(out,'</body>',UI_BLOCK+'\n</body>','index anchor');}
...
module.exports={SERVER_MARKER,ASSET_VERSION,UI_MARKER,DRAFT_SYNC_MARKER,patchServer,patchIndex,patchApp,apply};
