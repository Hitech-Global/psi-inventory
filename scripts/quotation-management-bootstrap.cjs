...
function patchIndexSource(src){if(src.includes(IMPORT_NORMALIZER_MARKER))return src;if(src.includes(INDEX_MARKER))return src.replace(INDEX_MARKER,INDEX_BLOCK);return once(src,'</body>',INDEX_BLOCK+'\n</body>','index anchor');}
...
module.exports={SERVER_MARKER,INDEX_MARKER,IMPORT_NORMALIZER_MARKER,patchServerSource,patchIndexSource,apply};
