...
function gmsPromotionRow({ shopId, campaignId, eventDate, performance, raw = {} }) {
  ...
  return {
    ...
    impressions: p.impressions, clicks: p.clicks, expense: p.expense, orders: p.broadOrders, gmv: p.broadGmv, sourceRoas: p.broadRoas, directGmv: p.directGmv, directRoas: p.directRoas, ctr: p.ctr, cvr: p.broadCvr,
...
function gmsItemRows(rows = []) {
  return rows.map(row => {
    ...
    return { itemId: Number(row.itemId ?? row.item_id), productName: row.productName ?? null, impressions: p.impressions, clicks: p.clicks, expense: p.expense, orders: p.broadOrders, gmv: p.broadGmv, sourceRoas: p.broadRoas, directGmv: p.directGmv, directRoas: p.directRoas, ctr: p.ctr, cvr: p.broadCvr, ...quality(p), raw: { source: 'GMS_ITEM', raw: row, directGmv: p.directGmv, directRoas: p.directRoas, sourceDirectGmvPresent: p.sourceDirectGmvPresent } };
...
function individualPromotionRow({ shopId, campaign, eventDate, performance, raw = {} }) {
  ...
  return {
    ...
    impressions: p.impressions, clicks: p.clicks, expense: p.expense, orders: p.broadOrders, gmv: p.broadGmv, sourceRoas: p.broadRoas, directGmv: p.directGmv, directRoas: p.directRoas, ctr: p.ctr, cvr: p.broadCvr, itemCount: 1, ...quality(p), raw: { source: 'PRODUCT_ADS', response: raw, directGmv: p.directGmv, directRoas: p.directRoas, sourceDirectGmvPresent: p.sourceDirectGmvPresent },
  };
}
