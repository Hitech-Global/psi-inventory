...
const API_CATALOG = Object.freeze({
  ...
  // Wave 0A：批量导入 Guardrails（行数硬上限 / pre-parse body 上限）
  "api.252": Object.freeze({ zh: "导入请求体过大（上限 {0}），请拆分后重试。", en: "Import request body too large (limit {0}); please split the import and retry.", id: "Isi permintaan impor terlalu besar (batas {0}); silakan pisahkan impor dan coba lagi." }),
  "api.253": Object.freeze({ zh: "本次导入共 {rows} 行，单次最多支持 {limit} 行，请拆分后重试。", en: "This import contains {rows} rows; a single request supports at most {limit} rows. Please split it and retry.", id: "Impor ini berisi {rows} baris; satu permintaan mendukung maksimal {limit} baris. Silakan pisahkan dan coba lagi." }),
