...
function list(){
  ...
  $('#qm-body').innerHTML=r.length?r.map(x=>`<tr data-s="${E(x.sku_code)}" data-b="${E(x.brand)}"><td><b>${E(x.sku_code)}</b></td><td>${E(x.brand)}</td><td>${E(x.product_type)}</td><td><b>${quoteMoney(x.latest_price,x.currency,x.quote_status)}</b></td><td>${E(x.currency)}</td><td>${E(x.latest_date)}</td><td>${change(x.vs_previous_pct)}</td><td>${change(x.vs_purchase_pct)}</td><td>${x.history_count}</td><td><span class="qm-link">查看</span></td></tr>`).join(''):'<tr><td colspan="10" class="qm-empty">暂无报价数据，请先导入</td></tr>';
...
function chart(points,ccy,field='avg_price'){
  ...
  const v=points.map(p=>Number(p[field])).filter(Number.isFinite);if(v.length<2)return '<div class="qm-empty">历史数据不足，暂无趋势图</div>';
  const min=Math.min(...v),max=Math.max(...v),w=520,h=180,p=22,span=max-min||1,xy=v.map((n,i)=>[p+(w-p*2)*(i/(v.length-1)),h-p-(h-p*2)*((n-min)/span)]),path=xy.map((q,i)=>(i?'L':'M')+q[0].toFixed(1)+' '+q[1].toFixed(1)).join(' ');
...
function brief(r){if(!r||!r.sku_count)return '当前品牌暂无报价数据。';const a=r.avg_change_pct==null?'暂无足够历史数据':`最新有效报价平均较上次${r.avg_change_pct>=0?'上涨':'下降'} ${Math.abs(r.avg_change_pct).toFixed(2)}%`,n=r.no_quote_count?`，其中 ${r.no_quote_count} 个 SKU 最新记录为未报价`:'';return `${r.brand||S.brand} 当前覆盖 ${r.sku_count} 个 SKU，共 ${r.history_count} 条历史记录${n}；${a}。与最近采购价可比的 SKU 中，${r.above_purchase_count} 个高于上次采购价，${r.below_purchase_count} 个低于上次采购价。`;}
...
function report(){
  ...
  $('#qm-right').innerHTML=`<div class="qm-report"><div class="qm-report-title"><div><div class="qm-brand">${E(r.brand||S.brand)} <span class="qm-badge">品牌报告</span></div><div class="qm-sub">自动生成 · 当前筛选口径</div></div><button class="qm-btn" id="qm-export">⇩ 导出品牌报告</button></div><div class="qm-kpis"><div class="qm-kpi"><span>SKU 总数</span><b>${r.sku_count||0}</b></div><div class="qm-kpi"><span>历史记录</span><b>${r.history_count||0}</b></div><div class="qm-kpi"><span>本月更新</span><b>${r.updated_this_month||0}</b></div><div class="qm-kpi"><span>当前未报价</span><b>${r.no_quote_count||0}</b></div></div><div class="qm-section"><h4>品牌报价趋势</h4>${chart(r.trend,r.currency)}</div><div class="qm-two qm-section"><div class="qm-mini"><h4>价格波动较大的 SKU</h4>${mini(r.top_movers,'vs_previous_pct')}</div><div class="qm-mini"><h4>较上次采购价差异</h4>${mini(r.top_purchase_gaps,'vs_purchase_pct')}</div></div><div class="qm-section"><h4>产品类型分布</h4>${Object.entries(r.product_types||{}).map(([k,v])=>`<div style="display:flex;justify-content:space-between;font-size:12px;margin:7px 0"><span>${E(k)}</span><b>${v}</b></div>`).join('')||'—'}</div><div class="qm-section qm-mini"><h4>品牌简报</h4><div style="font-size:13px;line-height:1.8;color:#555">${E(brief(r))}</div></div></div>`;
...
function renderSku(){
  const d=S.detail,h=d.history||[],l=h[0]||{},p=d.purchase,quotedHistory=h.filter(x=>x.quote_status!=='no_quote'&&x.quote_price!=null);
  $('#qm-right').innerHTML=`<div class="qm-report"><div class="qm-report-title"><div><div class="qm-brand">${E(d.sku_code)} <span class="qm-badge">${E(d.brand)}</span></div><div class="qm-sub">${E(l.product_type||'')}</div></div><button class="qm-btn" id="qm-back">← 返回品牌报告</button></div><div class="qm-kpis"><div class="qm-kpi"><span>最新 FOB</span><b>${quoteMoney(l.quote_price,l.currency,l.quote_status)}</b></div><div class="qm-kpi"><span>记录日期</span><b style="font-size:15px">${E(l.quote_date||'—')}</b></div><div class="qm-kpi"><span>较上次报价</span><b style="font-size:16px">${l.vs_previous_pct==null?'—':l.vs_previous_pct.toFixed(2)+'%'}</b></div><div class="qm-kpi"><span>较上次采购价</span><b style="font-size:16px">${l.vs_purchase_pct==null?'—':l.vs_purchase_pct.toFixed(2)+'%'}</b></div></div><div class="qm-section"><h4>FOB 价格趋势</h4>${chart([...quotedHistory].reverse(),l.currency,'quote_price')}</div><div class="qm-section"><h4>最近采购基准（CI）</h4><div class="qm-mini" style="font-size:12px">${p&&p.status==='exact'?`${E(p.ci_no)} · ${money(p.unit_price,p.currency)} · ${E(p.purchase_date||'')}`:p&&p.status==='ambiguous'?'最近 CI 同 SKU 存在不同原单价，已阻止比较':'暂无可比采购价'}</div></div><div class="qm-section"><h4>历史报价记录</h4><div class="qm-table-wrap" style="max-height:280px"><table class="qm-table"><thead><tr><th>报价日期</th><th>FOB</th><th>币种</th><th>较上一有效报价</th><th>备注</th></tr></thead><tbody>${h.map(x=>`<tr><td>${E(x.quote_date)}</td><td>${quoteMoney(x.quote_price,x.currency,x.quote_status)}</td><td>${E(x.currency)}</td><td>${change(x.vs_previous_pct)}</td><td>${E(x.remark||'—')}</td></tr>`).join('')}</tbody></table></div></div></div>`;
