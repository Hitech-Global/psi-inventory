...
function progressHtml(run){
  ...
  const counts=run&&terminal(run)?`新增 ${Number(run.inserted)||0} · 更新 ${Number(run.updated)||0} · 跳过 ${Number(run.skipped)||0} · 未报价 ${Number(run.no_quote)||0}`:(total?`${done} / ${total} 条`:'正在准备…');
  let extra='';
  if(run&&run.status==='completed')extra=`<div class="qip-ok">导入已完成。${counts}</div>`;
  else if(run&&(run.status==='failed'||run.status==='blocked')){
    const detail=(run.errors||[]).slice(0,6).map(e=>`第 ${e.row} 行：${e.reason}`).join('\n');
...
function renderModalProgress(run){
  ...
  const submit=$('#qm-submit',currentModal);if(submit){submit.disabled=!terminal(run);submit.textContent=terminal(run)?'完成':'正在导入 '+Math.round(Number(run.percent)||0)+'%';}
...
function refreshQuotation(){
  if($('#qm')&&typeof window.showQuotationManagement==='function')setTimeout(()=>window.showQuotationManagement(),80);
}

function finish(run){
  stopPolling();updateTopButton(run);renderModalProgress(run);
  if(run.status==='completed'){
    setId('');notify(`报价导入完成：新增 ${run.inserted||0}，更新 ${run.updated||0}，跳过 ${run.skipped||0}`,'success');refreshQuotation();
...
async function poll(id){
  if(!id)return;
  try{
    const run=await api('/api/quotation-management/import/'+encodeURIComponent(id)+'/status');
    missCount=0;updateTopButton(run);renderModalProgress(run);
    if(terminal(run)){finish(run);return;}
    pollTimer=setTimeout(()=>poll(id),450);
  }catch(e){
    if(e&&e.status===404)missCount++;else missCount=0;
    if(missCount>=5){stopPolling();setId('');updateTopButton(null);notify('未找到导入任务。若服务刚重启，请先检查报价列表是否已更新，再决定是否重试。','warning');return;}
    pollTimer=setTimeout(()=>poll(id),900);
  }
}
function stopPolling(){if(pollTimer){clearTimeout(pollTimer);pollTimer=null;}}
function startPolling(id){stopPolling();missCount=0;pollTimer=setTimeout(()=>poll(id),180);}
...
function openModal(){
  ...
  m.innerHTML=`<div class="qm-dialog"><h3>导入报价</h3><div class="qm-drop"><input type="file" id="qm-file" accept=".xlsx,.xls,.csv"><div class="qm-sub" style="margin-top:8px">字段：SKU、品牌、产品类型、FOB价格、币种、报价日期、备注（可选）</div><div class="qm-sub" style="margin-top:5px">FOB 填 0 = 当日未报价；会保留记录，但不会参与趋势或环比。</div></div><div style="margin-top:12px;font-size:13px">重复 SKU + 品牌 + 报价日期： <select id="qm-mode"><option value="skip">跳过已有记录</option><option value="overwrite">覆盖已有记录</option></select></div><div id="qm-preview" class="qm-sub" style="margin-top:8px"></div><div id="qm-import-live"></div><div class="qm-dialog-actions"><button class="qm-btn" id="qm-cancel">取消</button><button class="qm-btn primary" id="qm-submit">开始导入</button></div></div>`;
  document.body.appendChild(m);currentRows=[];
  const active=getId();
  if(active){$('#qm-file',m).disabled=true;$('#qm-mode',m).disabled=true;$('#qm-submit',m).disabled=true;$('#qm-preview',m).textContent='检测到未完成的导入任务，正在恢复状态…';renderModalProgress({status:'running',phase:'starting',percent:0});startPolling(active);}
  $('#qm-cancel',m).onclick=()=>{m.remove();currentModal=null;};
  $('#qm-file',m).onchange=async e=>{
    const f=e.target.files[0];if(!f)return;
    try{
      const wb=XLSX.read(await f.arrayBuffer(),{type:'array'});currentRows=XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]],{defval:''});
      $('#qm-preview',m).textContent='已读取 '+currentRows.length+' 行。导入采用批量写入；刷新页面后可恢复任务状态。';
    }catch(err){currentRows=[];$('#qm-preview',m).textContent='文件读取失败：'+(err.message||err);}
  };
  $('#qm-submit',m).onclick=async()=>{
    if(getId()){return;}
    if(!currentRows.length)return alert('请先选择文件');
    const id=makeId();setId(id);
    $('#qm-file',m).disabled=true;$('#qm-mode',m).disabled=true;$('#qm-submit',m).disabled=true;
    const initial={import_id:id,status:'running',phase:'starting',percent:1,total_count:currentRows.length,processed_count:0,message:'正在启动报价导入'};
    updateTopButton(initial);renderModalProgress(initial);startPolling(id);
    try{
      const res=await api('/api/quotation-management/import','POST',{rows:currentRows,duplicate_mode:$('#qm-mode',m).value,import_id:id});
      if(terminal(res))finish(res);else{updateTopButton(res);renderModalProgress(res);}
    }catch(e){
      if(String(e&&e.message||'').startsWith('导入校验失败')){
        stopPolling();setId('');updateTopButton(null);renderModalProgress({status:'blocked',phase:'blocked',percent:0,message:e.message||'导入校验失败'});
      }else{
        const p=$('#qm-preview',m);if(p)p.textContent='导入请求暂未返回，正在通过任务 ID 自动确认结果，请勿重复提交。';
      }
    }
  };
}
...
function resumeIfNeeded(){const id=getId();if(id){updateTopButton({status:'running',percent:0});startPolling(id);}}
