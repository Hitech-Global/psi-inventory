...
async function initDatabase() {
  ...
  try { await exec(`ALTER TABLE sales_records ADD COLUMN IF NOT EXISTS country TEXT DEFAULT ''`); } catch (e) { /* 列已存在 */ }
  ...
  await exec(`CREATE INDEX IF NOT EXISTS idx_sales_records_country ON sales_records(country)`);
  ...
  `);
  await exec(`CREATE INDEX IF NOT EXISTS idx_sales_import_runs_status ON sales_import_runs(status)`);
  await exec(`CREATE INDEX IF NOT EXISTS idx_sales_import_runs_updated ON sales_import_runs(updated_at)`);
  ...
  await exec(`
    CREATE TABLE IF NOT EXISTS replenishment_suggestions (
      ...
      with_transit_turnover_months DOUBLE PRECISION DEFAULT 0,
      with_pi_turnover_months DOUBLE PRECISION DEFAULT 0,
      with_po_turnover_months DOUBLE PRECISION DEFAULT 0,
      manual_planned_qty INTEGER DEFAULT 0,
      online_sales_m1 DOUBLE PRECISION DEFAULT 0,
      online_sales_m2 DOUBLE PRECISION DEFAULT 0,
      online_sales_m3 DOUBLE PRECISION DEFAULT 0,
      online_sales_m4 DOUBLE PRECISION DEFAULT 0,
      offline_sales_m1 DOUBLE PRECISION DEFAULT 0,
      offline_sales_m2 DOUBLE PRECISION DEFAULT 0,
      offline_sales_m3 DOUBLE PRECISION DEFAULT 0,
      offline_sales_m4 DOUBLE PRECISION DEFAULT 0,
      online_avg_sales_4m DOUBLE PRECISION DEFAULT 0,
  ...
  await exec(`
    CREATE TABLE IF NOT EXISTS payable_items (
      ...
      source_ci_id TEXT NOT NULL DEFAULT '',
      created_by TEXT DEFAULT '',
      created_at TEXT DEFAULT NOW(),
